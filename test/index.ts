...
describe('Airdrop tests', function () {
 ...
 let _airdropContract: Airdrop;
	...
	it('Should claim', async function () {
		...
		expect(await _airdropContract.connect(_taxExcluded).claim()).to.emit(_airdropContract, 'eventClaimed').withArgs(_taxExcluded.address, airdropAmount);
	...
	it('Should burn remaining tokens', async function () {
		...
		await _airdropContract.claim();
		await ethers.provider.send('evm_increaseTime', [airdropTime * 60]);
		expect(await _airdropContract.burnRemainingTokens()).to.emit(_airdropContract, 'eventBurnRemainingTokens').withArgs(airdropAmount);
...
describe('Presale tests', function () {
	...
	it('Should deposit', async function () {
		...
		await _presaleContract.depositOwn(presaleDepositOwnAmount);
		await _stablecoinContract.approve(_presaleContract.address, presaleDepositAmount);
		expect(await _presaleContract.deposit(presaleDepositAmount)).to.emit(_presaleContract, 'eventDeposited').withArgs(_owner.address, presaleDepositAmount);
	...
	it('Should burn remaining tokens', async function () {
		...
		expect(await _presaleContract.burnRemainingTokens()).to.emit(_presaleContract, 'eventBurnRemainingTokens').withArgs(presaleDepositOwnAmount);
	...
	it('Should claim', async function () {
		...
		await _presaleContract.deposit(presaleDepositAmount);
		await ethers.provider.send('evm_increaseTime', [presaleDepositTimeInMinutes * 60]);
		expect(await _presaleContract.claim()).to.emit(_presaleContract, 'eventClaimed').withArgs(_owner.address, presaleDepositAmount);
...
describe('Pool tests', function () {
	...
	it('Should deposit with no fee', async function () {
		...
		expect(await _poolContract.deposit(poolOurId, poolTokensDeposit)).to.emit(_poolContract, 'eventDeposit').withArgs(_owner.address, poolOurId, poolTokensDeposit);
	...
	it('Should deposit with fee', async function () {
		...
		expect(await _poolContract.deposit(poolUSDId, poolTokensDeposit)).to.emit(_poolContract, 'eventDeposit').withArgs(_owner.address, poolUSDId, poolTokensDeposit);
	...
	it('Should withdraw with 0 amount', async function () {
		...
		expect(await _poolContract.withdraw(poolOurId, 0)).to.emit(_poolContract, 'eventWithdraw').withArgs(_owner.address, poolOurId, amountToWithdraw);
	...
	it('Should withdraw with deposit amount', async function () {
		...
		await ethers.provider.send('evm_mine', []);
		const amountToWithdraw = await _poolContract.pendingTokens(poolOurId, _owner.address);
		expect(await _poolContract.withdraw(poolOurId, poolTokensDeposit)).to.emit(_poolContract, 'eventWithdraw').withArgs(_owner.address, poolOurId, amountToWithdraw);
		const postBlockNumber = await ethers.provider.getBlockNumber();
		const multiplier = postBlockNumber - preBlockNumber;
	...
	it('Should emergency withdraw', async function () {
		...
		await _poolContract.deposit(poolOurId, poolTokensDeposit);
		await ethers.provider.send('evm_mine', []);
		expect(await _poolContract.emergencyWithdraw(0)).to.emit(_poolContract, 'eventEmergencyWithdraw').withArgs(_owner.address, poolOurId, poolTokensDeposit);
