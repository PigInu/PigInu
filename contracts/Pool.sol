...
contract Pool is Ownable, ReentrancyGuard {
    ...
    function deposit(uint256 _poolID, uint256 _amount) public nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending = user
                ...
                .sub(user.rewardDebt);
            if (pending > 0)
                safeTokenTransfer(address(tokenEarn), msg.sender, pending);
        }
        if (_amount > 0) {
            pool.tokenDeposit.safeTransferFrom(
                msg.sender,
                address(this),
                _amount
            );
    ...
    function createPool(
        ...
        pools.push(
            PoolInfo({
                tokenDeposit: _lpToken,
                allocPoint: _allocPoint,
                lastRewardBlock: lastRewardBlock,
                accTokenPerShare: 0,
                feeDeposit: _depositFeeBP
            })
        );
    }
    ...
    function burnRemainingTokens() external onlyOwner {
        ...
        tokenEarn.safeTransferFrom((address(this)), burnAddress, rewardTokensLeft);
    }
